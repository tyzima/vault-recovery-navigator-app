...
>(({ className, children, ...props }, ref) => {
  const isSuccess = typeof children === 'string' && children.toLowerCase().includes('success')
  
  return (
    <ToastPrimitives.Title
      ref={ref}
      className={cn("text-sm font-medium text-left flex items-center gap-2", className)}
      {...props}
    >
      {isSuccess && <Check className="h-4 w-4 text-green-600" />}
      {children}
    </ToastPrimitives.Title>
  )
})
...
>(({ className, ...props }, ref) => (
  <ToastPrimitives.Description
    ref={ref}
    className={cn("text-sm opacity-80 text-left", className)}
    {...props}
  />
))
